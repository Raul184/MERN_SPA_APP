...
export function* fetchDataAsync() {
  try {
    ...
    yield put(fetchSuccess(all, data));
  } catch (error) {
    yield put(fetchFailed(all, error.message));
  }
}
// Listener
...
export function* fetchBookedAsync() {
  try {
    ...
    yield put(grabBookedToursSuccess(data.data.tours));
  } catch (error) {
    yield put(grabBookedToursFailed(error.message));
  }
}
// Listener
...
export function* toursSagas() {
  yield all([call(fetchStart), call(fetchBookedStart)]);
}
